...
On Windows, install VS Code plus its WSL extension and run `code .` from inside
the project. You get a Windows editor writing directly to the files in WSL,
with an Ubuntu terminal built in. Keep the project in your Linux home
directory, not under `/mnt/c` — Git is much slower there and reports spurious
...
### Sections you have not filled in yet

Empty sections remove themselves. You do not need to invent content to avoid a
bare heading, and you do not need to delete anything defensively:

| If this is empty | What visitors see |
| --- | --- |
| A role group in `team.yml` with no people | The group heading does not appear |
| `news.yml` | No News column; publications take the full width |
| `publications.bib` | The Publications nav link disappears |
| `projects.yml` | The Research nav link disappears |
| No entry marked `selected = {true}` | The homepage publications block is skipped |
| `join_note` deleted from `site.yml` | The recruiting box at the bottom goes away |

So you have three equally safe options for a group you do not have yet, such as
undergraduates: leave the heading in place with no `people:` under it, comment
it out, or delete it. Nothing renders either way. Deleting is cleanest, and Git
keeps the old version if you want it back — but pasting the block in again from
this README is faster than digging through history.

The publication type filters only appear once there are five or more papers.
